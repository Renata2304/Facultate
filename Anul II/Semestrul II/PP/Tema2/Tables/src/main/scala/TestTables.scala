...
object TestTables {
  ...
  def programmingLanguages1: Table = {
    val table1 = NewCol("Object-Oriented", "Yes", Value(tableObjectOriented))
    val table2 = NewCol("Functional", "Yes", Value(tableFunctional))
    val table3 = NewCol("Imperative", "Yes", Value(tableImperative))
    Merge("Language", table2, Merge("Language", table1, table3)).eval.get
  }

  // 3.2
  val programmingLanguages2: Table = {
    Filter(And(Field("Original purpose", _.contains("Application")),
      Field("Other paradigms", _.contains("concurrent"))), Value(programmingLanguages1)).eval.get
  }

  // 3.3
  val programmingLanguages3: Table = {
    val newVList = List("Language", "Object-Oriented", "Functional")
    Select(newVList, Value(programmingLanguages2)).eval.get
  }
}
